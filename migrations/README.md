...
- Apply with `psql -f migrations/001_create_airports.sql "$DATABASE_URL"`; ensure the role has privileges before running.
...
- Apply with `psql -f migrations/002_create_airlines.sql "$DATABASE_URL"` after ensuring the role has `USAGE` and `CREATE` on the target schema.
...
- Apply with `psql -f migrations/003_create_aircrafts.sql "$DATABASE_URL"` once the role has `USAGE` and `CREATE` on the schema.
...
- Apply with `psql -f migrations/004_create_countries.sql "$DATABASE_URL"` after confirming schema privileges.
...
- Grant schema and database privileges needed for migrations and integration tests:
  ```sql
  GRANT CREATE ON DATABASE flight_data TO codex;
  GRANT USAGE, CREATE ON SCHEMA public TO codex;
  ```
